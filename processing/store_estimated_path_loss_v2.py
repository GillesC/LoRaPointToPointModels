...
with open(os.path.join(path_to_measurements, "measurements.json")) as f:
    ...
    for measurement in measurements:
        ...
        sf_censored = df["sf"].values
        c_all = np.array([sf_pl_th_mapping[sf] for sf in sf_censored])
        ...
        c_uncensored = np.array([sf_pl_th_mapping[sf] for sf in sf_uncensored])
        ...
        for (i, (sigma, sigma_name)) in enumerate(zip([sigma_ols], ["constant"])):
            ...
            for weight_type, weight_name in zip([None, w_lin, w_log, w_sq], ["No", "Linear", "Log10", "Square"]):
                ...
                ml_value = model.ml_value(pld=pld_uncensored, sigma_est=sigma_est, plm_est=plm_est, c=c_uncensored)
        ...
        for (i, (sigma, sigma_name)) in enumerate(
            ...
            for weight_type, weight_name in zip([None, w_lin_all, w_log_all, w_sq_all],
                ...
                res = model.ml_dual_slope_with_constraints(d0=d0, d=d_all, pld=pld_all, c=c_all, x0=x0,
                                                           weights=weight_type,
